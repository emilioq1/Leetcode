...
int** fourSum(int* nums, int numsSize, int target, int* returnSize,
	...
	*returnSize = 0;
	...
	for(int i = 0; i < numsSize - 3; ++i) {
		...
		// Pruning for i: min possible sum with this i already exceeds target
		if((long long)nums[i] + nums[i + 1] + nums[i + 2] + nums[i + 3] >
		   target)
			break;

		// Pruning for i: max possible sum with this i still falls short
		if((long long)nums[i] + nums[numsSize - 3] + nums[numsSize - 2] +
			   nums[numsSize - 1] <
		   target)
			continue;
		...
		for(int j = i + 1; j < numsSize - 2; ++j) {
			...
			// Pruning for j: min possible sum with this (i,j) exceeds target
			if((long long)nums[i] + nums[j] + nums[j + 1] + nums[j + 2] >
			   target)
				break;
			// Pruning for j: max possible sum with this (i,j) falls short
			if((long long)nums[i] + nums[j] + nums[numsSize - 2] +
				   nums[numsSize - 1] <
			   target)
				continue;
